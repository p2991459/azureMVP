...
class TextEmbedder():
    ...
    def embed_content(self, text, clean_text=True, use_single_precision=True):
        ...
        logging.info(f"Generating embedding of text: {text} using engine: {self.AZURE_OPENAI_EMBEDDING_DEPLOYMENT}")
        response = openai.Embedding.create(input=text, engine=self.AZURE_OPENAI_EMBEDDING_DEPLOYMENT)
        logging.info(f"Got the response from embedding model here is the response: {response}")
        embedding = [round(x, embedding_precision) for x in response['data'][0]['embedding']]
        logging.info(f"Successfully returned embedding: {embedding}")
        return embedding